...
export default function ExperienceCounter() {
  ...
  useEffect(() => {
    const calculateYears = () => {
      ...
      const codingYearsCalc = Math.floor(
        codingDiff / (1000 * 60 * 60 * 24 * 365.25),
      );

      const professionalDiff = now.getTime() - professionalStart.getTime();
      const professionalYearsCalc = Math.floor(
        professionalDiff / (1000 * 60 * 60 * 24 * 365.25),
      );
  ...
  return (
    <span className="font-bold text-ink">
      {codingYears}+ years of coding and {professionalYears}+ years of
